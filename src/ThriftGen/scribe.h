...
namespace scribe {
    namespace thrift {
        ...
        public:
            ...
            virtual void LogAndForget(const std::vector<LogEntry> & messages) = 0;
        };
        ...
        public:
            ...
            void LogAndForget(const std::vector<LogEntry> & /* messages */) {
                return;
            }
        };
        ...
        typedef struct _scribe_LogAndForget_args__isset {

            _scribe_LogAndForget_args__isset() : messages(false) {
            }
            bool messages;
        } _scribe_LogAndForget_args__isset;

        class scribe_LogAndForget_args {
        public:

            scribe_LogAndForget_args() {
            }

            virtual ~scribe_LogAndForget_args() throw () {
            }

            std::vector<LogEntry> messages;

            _scribe_LogAndForget_args__isset __isset;

            void __set_messages(const std::vector<LogEntry> & val) {
                messages = val;
            }

            bool operator==(const scribe_LogAndForget_args & rhs) const {
                if (!(messages == rhs.messages))
                    return false;
                return true;
            }
            ...
            bool operator<(const scribe_LogAndForget_args &) const;
        ...
        class scribe_LogAndForget_pargs {
        public:

            virtual ~scribe_LogAndForget_pargs() throw () {
            }
        ...
        public:
            ...
            ResultCode recv_Log();
            void LogAndForget(const std::vector<LogEntry> & messages);
            void send_LogAndForget(const std::vector<LogEntry> & messages);
        };
        ...
        private:
            ...
            void process_LogAndForget(int32_t seqid, ::apache::thrift::protocol::TProtocol* iprot, ::apache::thrift::protocol::TProtocol* oprot, void* callContext);
        public:
            ...
            iface_(iface) {
                ...
                processMap_["LogAndForget"] = &scribeProcessor::process_LogAndForget;
            }
        ...
        public:
            ...
            void LogAndForget(const std::vector<LogEntry> & messages) {
                ...
                size_t i = 0;
                for (; i < (sz - 1); ++i) {
                    ifaces_[i]->LogAndForget(messages);
                }
                ifaces_[i]->LogAndForget(messages);
            }
